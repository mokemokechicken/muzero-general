...
MAX_NUMBER = 16  # means 2^16. if 2^17 is created, game finish.
BOARD_SIZE = 4
...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (MAX_NUMBER, BOARD_SIZE, BOARD_SIZE)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Make2048:
    ACTIONS = ("r", "u", "l", "d")  # Right, Up, Left, Down
    ...
    def get_observation(self):
        ...
        for k in range(1, MAX_NUMBER+1):
            ch = numpy.where(self.board == k, 1, 0)
            channels.append(ch)
    ...
    def human_to_action(self):
        while True:
            try:
                ch = input(f"Enter [u]Up, [d]Down, [l]Left, [r]Right: ").strip()
                if ch in self.ACTIONS:
                    choice = self.ACTIONS.index(ch)
                    break
            except:
                pass
